...
export class Revenue {
    ...
    processCategories() {
        ...
        if (this.categories && this.categories.length > 0) {
            this.categories.forEach(item => {
                ...
                blockEditElement.onclick = () => {
                    const userEditRevenue = {
                        id: item.id,
                        title: item.title
                    }
                    window.location.href = '#/edit-revenue'
                    localStorage.setItem('userEditRevenue', JSON.stringify(userEditRevenue));
                }
                ...
                blockDeleteElement.onclick = () => {
                    ...
                    deleteCategory.onclick = async () => {
                        try {
                            myModal.hide();
                            const result = await CustomHttp.request(config.host + '/categories/income/' + item.id, 'DELETE');
                            if (result) {
                                if (result.error) {
                                    throw new Error(result.error);
                                }
        ...
        createCategory.onclick = () => {
            window.location.href = '#/create-revenue'
        }
    }
}